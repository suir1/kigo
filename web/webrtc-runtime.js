(function installKigoWebRTC(global) {
  ...
  function create(options = {}) {
    ...
    const hostTimeoutMs = options.hostTimeoutMs || 2500;
    ...
    async function runPeerSession(config, handler) {
      ...
      for (let attempt = 1; attempt <= reconnectAttempts; attempt++) {
        ...
        try {
          const iceMode = attemptICEPolicy(Boolean(config.directFirst), attempt);
          const connectTimeoutMs = iceMode === "host"
            ? hostTimeoutMs
            : iceMode === "direct" ? directTimeoutMs : signalTimeoutMs;
          const connectStarted = performance.now();
          const useParallelData = Boolean(config.parallelData) && iceMode !== "relay";
          const useUnorderedData = Boolean(config.unorderedData) && !useParallelData;
          peer = await createPeer(role, token, task, protocol, iceMode, useUnorderedData);
          const primaryClose = peer.close;
          const dataPeers = [];
          peer.pcs = [peer.pc];
          peer.signalTokens = [token];
          peer.close = () => {
          ...
          if (useParallelData) {
            try {
              for (let lane = 1; lane <= 2; lane++) {
                const laneToken = await parallelLaneToken(token, lane);
          ...
          if (!shouldRetry(err, peer?.signal, attempt, task)) throw err;
          await onRetry({
            err,
            attempt,
            nextAttempt: attempt + 1,
            nextMode: attemptICEPolicy(Boolean(config.directFirst), attempt + 1),
            maxAttempts: reconnectAttempts,
            diagnostics: peer?.iceDiagnostics?.(),
          });
  ...
  function attemptICEPolicy(directFirst, attempt) {
    if (!directFirst) return "all";
    if (attempt === 1) return "host";
